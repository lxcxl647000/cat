 ...
 * Converted from JavaScript version by Paul Johnston.
 */
    ...
    for (let i = 0; i < binarray.length * 32; i += 6) {
        str += tab.charAt(
            (((binarray[i >> 5] << (i % 32)) & 0x3f) |
                ((binarray[(i >> 5) + 1] >>> (32 - (i % 32))) & 0x3f)) & 0x3f
        );
    }
    return str;
}
...
function strw2binl(str: string): number[] {
    const nblk = ((str.length + 4) >> 5) + 1;
    const blks = new Array(nblk * 16).fill(0);
    let i: number;
    for (i = 0; i < str.length; i++) {
...
export function hexMD5w(str: string): string {
    return binl2hex(coreMD5(strw2binl(str)));
}

export function b64MD5(str: string): string {
    return binl2b64(coreMD5(str2binl(str)));
}

export function b64MD5w(str: string): string {
