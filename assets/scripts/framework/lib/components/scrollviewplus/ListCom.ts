...
import ListComItem from './ListComItem';
...
export default class ListCom extends Component {
    ...
    set selectedId(val: number) {
        ...
        switch (t.selectedMode) {
            case SelectedType.SINGLE: {
                ...
                t._selectedId = val;
                if (item) {
                    listItem = item.getComponent(ListComItem);
                    listItem.selected = true;
                }
                if (t._lastSelectedId >= 0 && t._lastSelectedId != t._selectedId) {
                    let lastItem: any = t.getItemByListId(t._lastSelectedId);
                    if (lastItem) {
                        lastItem.getComponent(ListComItem).selected = false;
                    }
                }
                if (t.selectedEvent) {
            ...
            case SelectedType.MULT: {
                ...
                if (!item)
                    return;
                let listItem = item.getComponent(ListComItem);
                if (t._selectedId >= 0)
                    t._lastSelectedId = t._selectedId;
                t._selectedId = val;
    ...
    setTemplateItem(item: any) {
        ...
        let com: any = item.getComponent(ListComItem);
        let remove = false;
        if (!com)
            remove = true;
        // if (com) {
    ...
    _onTouchStart(ev, captureListeners) {
        ...
        let eventType = 2;
        // if (Event && Event.AT_TARGET) {
        //     eventType = Event.AT_TARGET;
        // }
        let isMe = ev.eventPhase === eventType && ev.target === this.node;
        if (!isMe) {
    ...
    _createOrUpdateItem(data: any) {
        ...
        if (!item) { //如果不存在
            ...
            let listItem: ListComItem = item.getComponent(ListComItem);
    ...
    _updateListItem(listItem: ListComItem) {
        if (!listItem)
            return;
